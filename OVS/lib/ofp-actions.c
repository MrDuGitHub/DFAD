...
/* handle gtp actions. */ 
static enum ofperr 
decode_OFPAT_RAW10_MARK(struct ofpbuf *out) 
{ 
    ofpact_put_MARK(out)->ofpact.raw = OFPAT_RAW10_MARK; 
    return 0; 
} 
 
static enum ofperr 
decode_OFPAT_RAW11_MARK(struct ofpbuf *out) 
{ 
    ofpact_put_MARK(out)->ofpact.raw = OFPAT_RAW11_MARK; 
    return 0; 
} 
 
static enum ofperr 
decode_OFPAT_RAW12_MARK(struct ofpbuf *out) 
{ 
    ofpact_put_MARK(out)->ofpact.raw = OFPAT_RAW12_MARK; 
    return 0; 
} 
 
static void 
encode_MARK(const struct ofpact_null *null OVS_UNUSED, 
                  enum ofp_version ofp_version, struct ofpbuf *out) 
{ 
    if (ofp_version == OFP10_VERSION) { 
        put_OFPAT10_MARK(out); 
    } else if (ofp_version == OFP11_VERSION) { 
        put_OFPAT11_MARK(out); 
    } else { 
        put_OFPAT12_MARK(out); 
    } 
} 
 
static char * OVS_WARN_UNUSED_RESULT 
parse_MARK(const char *arg,
             const struct ofputil_port_map *port_map,
             struct ofpbuf *ofpacts,
             enum ofputil_protocol *usable_protocols OVS_UNUSED) 
{ 
    ofpact_put_MARK(ofpacts)->ofpact.raw = OFPAT_RAW10_MARK; 
    return NULL; 
} 
 
static void 
format_MARK(const struct ofpact_output *a,const struct ofputil_port_map *port_map, struct ds *s)
{ 
    ds_put_cstr(s, "mark"); 
}
